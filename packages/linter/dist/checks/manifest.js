...
import { MANIFEST_NAME_PATTERN, MANIFEST_NAME_MIN_LENGTH, MANIFEST_NAME_MAX_LENGTH, CONFIG_TYPES, } from '../types.ts';
 ...
 */
export function parseAndValidateManifest(yamlContent) {
    let parsed;
    try {
...
// --- Field validators ---
function validateName(m, findings) {
...
function validateVersion(m, findings) {
...
function validateDescription(m, findings) {
...
function validateAuthor(m, findings) {
...
function validateCapabilities(m, findings) {
...
function validateConfig(m, findings) {
    if (!('config' in m) || !m.config)
        return;
    ...
    for (const field of m.config) {
        ...
        else if (!CONFIG_TYPES.includes(f.type)) {
            findings.push({
                severity: 'error',
                check: 'manifest/config',
...
function validateTags(m, findings) {
    if (!('tags' in m) || !m.tags)
        return;
...
function validateCategory(m, findings) {
...
function validateTrustLevel(m, findings) {
    if (!('trust_level' in m) || !m.trust_level)
        return;
...
function validateFrameworks(m, findings) {
    if (!('frameworks' in m) || !m.frameworks)
        return;
